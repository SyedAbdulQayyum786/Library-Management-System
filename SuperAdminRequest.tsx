...
import { allrequestofsuperadmin } from "../state/selectors/loginvalue";
import { requestofreader, requestsatsuperadmin, AllBooks } from "../state/atoms/loginstate";
...
const SuperAdminRequest: React.FC = () => {
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [getupdatedrequest, setupdatedrequest] = useRecoilState<Requests[]>(requestsatsuperadmin);
  const getallrequests = useRecoilValue(allrequestofsuperadmin);
  const [getreadersrequest, setreadersrequest] = useRecoilState<Requests[]>(requestofreader);
  ...
  const [books, setBooks] = useRecoilState(AllBooks);
  ...
  const handleStatusChange = (id: number, newStatus: string) => {
    ...
    setreadersrequest(prevRequests =>
      prevRequests.map(request =>
        request.id === id ? { ...request, requeststatus: newStatus } : request
      )
    );

    const updatedRequest = getupdatedrequest.find(request => request.id === id);

    if (updatedRequest && newStatus === "Completed") {
      setBooks(prevBooks =>
        prevBooks.map(book =>
          book.title === updatedRequest.bookname
